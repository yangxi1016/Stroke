...
result = []
output = sys.stdout
outputfile = open('sen-CPM-ING.txt', 'w', encoding='UTF-8')
sys.stdout = outputfile
f = open(r"../../sen-cpm.txt", "r", encoding='UTF-8')
a = open(r"../../../03ing-0127-f-EN-0.txt", "r", encoding='UTF-8')
lines = f.readlines()
lists = []
lines_a = a.readlines()
lists_a = []
            ...
            if (int(lists[i][2]) <= int(lists_a[j][1])) & (int(lists[i][3]) >= int(lists_a[j][2])):
                outputfile.write(
                    str(lists[i][0]) + "\t" + "sen_"+lists[i][1] + "\t" + lists[i][2] + "-" + lists[i][3] + "\t" + str(
                        ...
                        int(lists_a[j][2]) - int(lists[i][2])) + "###" + "Ingredients" + "###" + str(lists_a[j][3]) + "###" + str(int(lists_a[j][1]) - int(lists[i][2])) + "-" + str(
